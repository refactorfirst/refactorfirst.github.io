...
let spriteTextPromise = null;

function loadSpriteText() {
    spriteTextPromise ||= import('https://esm.sh/three-spritetext')
...
export async function initWasmGraphs(data) {
    try {
        const {init, DotParser} = await import(
            'https://cdn.jsdelivr.net/npm/@vizdom/vizdom-ts-web@0.1.19/vizdom_ts.min.js'
            ).then(m => ({init: m.init || m.default, DotParser: m.DotParser}));
        await init();
