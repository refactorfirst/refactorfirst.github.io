...
/**
 * Injects the trusted filter input and clear button for a rendered table.
 *
 * @param {Element} slot - Toolbar slot that receives the controls.
 * @param {string} tableId - Stable table identifier.
 * @param {string} caption - Human-readable table caption.
 * @param {string} currentTerm - Current filter value.
 * @param {number} debounceMs - Input debounce delay.
 * @param {Function} onTableAction - State-change callback.
 */
...
/** Binds sort buttons for a table to state updates. */
...
/** Binds previous and next controls for a paginated table. */
...
/** Binds CSV export controls using the table's current filter and sort state. */
...
/** Toggles horizontal scrolling only on table wrappers that overflow. */
...
/** Keeps headers viewport-sticky when their table wrapper scrolls horizontally. */
...
/** Installs the shared resize and scroll listeners and targets the latest root. */
...
/** Makes body cells keyboard-operable copy targets with result feedback. */
